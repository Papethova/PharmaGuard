...
import { MASTER_ADMIN_EMAIL, SCHEDULES } from "./lib/constants";
...
export default function App() {
  ...
  const isMasterAdmin = useMemo(() => user?.email?.toLowerCase() === MASTER_ADMIN_EMAIL, [user]);
  ...
  useEffect(() => {
    ...
    // Hard Bypass: If auth hasn't responded in 8 seconds, force show the login screen
    // This prevents the "Infinite Spinner" or "White Screen" if Firebase Auth is blocked
    ...
    const unsubAuth = onAuthStateChanged(auth, async (currentUser) => {
      ...
      try {
        if (currentUser && currentUser.email) {
          ...
          // Identity Bridge: If profile exists under UID but not email, migrate it immediately
          // Using a faster getDoc check
          let userDoc;
          try {
            userDoc = await getDoc(userDocRef);
          } catch (e) {
            console.warn("Initial user doc fetch failed, might not exist yet:", e);
          }
          ...
          if (!userDoc || !userDoc.exists()) {
            try {
              ...
              if (legacyDoc.exists()) {
                ...
                await setDoc(userDocRef, { ...legacyData, migratedFromUid: currentUser.uid, status: 'active' });
                userDoc = await getDoc(userDocRef);
              }
            } catch (e) {
          ...
          if (!userDoc || !userDoc.exists()) {
            const isMaster = currentUser.email.toLowerCase() === MASTER_ADMIN_EMAIL;
            ...
            try {
              ...
              if (e instanceof Error && e.message.includes("permission")) {
                toast.error("Security Notice: Profile creation rejected. Please contact support.");
              }
            }
          }

          // Real-time profile listener on the stable email ID
  ...
  useEffect(() => {
    ...
    // Listen to strictly the primary Email-based path (Unified Node)
    ...
    const unsubSubstances = onSnapshot(substancesRef, (snapshot) => {
      ...
      if (userProfile?.status === 'active') {
        handleFirestoreError(error, OperationType.LIST, `users/${emailId}/substances`);
      } else {
    ...
    const unsubTransactions = onSnapshot(query(transactionsRef, orderBy("timestamp", "desc")), (snapshot) => {
      ...
      if (userProfile?.status === 'active') {
        handleFirestoreError(error, OperationType.LIST, `users/${emailId}/transactions`);
      } else {
    ...
    const unsubStaff = onSnapshot(staffRef, (snapshot) => {
      ...
      if (userProfile?.status === 'active') {
        handleFirestoreError(error, OperationType.LIST, `users/${emailId}/staff`);
      } else {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(usersRef, (snapshot) => {
      const items = snapshot.docs.map(doc => {
        return { ...doc.data(), docId: doc.id } as UserProfile;
      });
  ...
  const handleLogTransaction = async () => {
    ...
    try {
      ...
      const emailId = user.email?.toLowerCase();
      if (!emailId) throw new Error("Unified node missing");
  ...
  const filteredUserProfiles = useMemo(() => {
    return allUserProfiles
      .filter((p: UserProfile) => 
        ...
        (p.displayName?.toLowerCase() || "").includes(userSearchTerm.toLowerCase()) ||
        (p.organizationName?.toLowerCase() || "").includes(userSearchTerm.toLowerCase())
      )
  ...
  const masterAccountProfile = useMemo(() => 
    filteredUserProfiles.find((p: UserProfile) => p.email?.toLowerCase() === MASTER_ADMIN_EMAIL),
  ...
  const handleUpdateOrgProfile = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, "users", emailId), { 
        ...
        displayName: editingOrgName.trim() // Also update display name for consistency
      });
        ...
        >
          <Card className="border-brand-blue/10 shadow-2xl bg-brand-surface overflow-hidden">
            <div className="bg-brand-blue p-6 text-center relative overflow-hidden">
              ...
              <p className="text-brand-yellow/80 font-bold text-[10px] uppercase tracking-widest mt-1">Registry Terminal v2.1</p>
            </div>
          ...
          <div className="bg-white p-8 rounded-2xl shadow-xl border border-brand-blue/20 flex flex-col items-center space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="bg-brand-blue/5 p-4 rounded-lg border border-brand-blue/10 mt-4">
                ...
                <p className="text-xs font-mono mt-1 text-brand-dark-grey pointer-events-none">{userProfile.email}</p>
              </div>
  ...
  return (
        ...
        >
            ...
            <div className="flex flex-col gap-3 w-full shrink-0">
              <div className="px-5 p-0 m-0 text-center flex flex-col items-center justify-center min-h-[40px]">
                <h3 className={`font-black text-blue-400/90 tracking-tight leading-tight transition-colors duration-300 ${
                  (userProfile?.organizationName?.length || 0) > 20 ? "text-lg" : 
                  (userProfile?.organizationName?.length || 0) > 15 ? "text-xl" : "text-2xl"
                }`}>
                  {userProfile?.organizationName || "Clinical Group"}
                </h3>
              </div>
              <Button 
              ...
              {user && (
                <div className="flex items-center justify-between p-3 rounded-lg bg-brand-blue/5 border border-brand-blue/10 group mt-8">
                  <div className="flex flex-col overflow-hidden">
                    ...
                    <span className="text-[10px] text-brand-blue/60 font-bold truncate pointer-events-none">{user.email}</span>
                  </div>
          ...
          <div className="w-full relative min-w-0 flex-1 z-10">
            ...
            <Dialog open={isLogOpen} onOpenChange={setIsLogOpen}>
               <DialogContent className="sm:max-w-[500px] bg-brand-surface border-brand-blue/20 shadow-2xl p-0 overflow-hidden rounded-2xl flex flex-col max-h-[90vh]">
                  ...
                  {transactionType !== "VERIFY" && (
                    <div className="grid gap-1.5">
                      ...
                      <Input 
                        ...
                        className={`border-brand-grey/20 focus-visible:ring-brand-blue bg-brand-surface text-brand-dark-grey h-9 ${transactionType === "ADJUST" ? "opacity-70 cursor-not-allowed" : ""}`}
            ...
            <Dialog open={!!viewingTransaction} onOpenChange={(open) => !open && setViewingTransaction(null)}>
              <DialogContent className="sm:max-w-[500px] bg-brand-surface border-brand-blue/10 p-0 overflow-hidden rounded-2xl flex flex-col max-h-[90vh]">
                  ...
                  <div className="p-6 space-y-8">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <Button onClick={handleAddUser} className="bg-brand-blue text-white hover:bg-brand-blue/90 h-8 px-4">
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {isInitializing ? (
                        ...
                        <TableRow>
                          <TableCell colSpan={5} className="text-center py-8 text-brand-dark-grey/50">No substances found.</TableCell>
                        </TableRow>
    ...
    <Dialog open={isSuperAdminOpen} onOpenChange={setIsSuperAdminOpen}>
      <DialogContent className="max-w-[95vw] lg:max-w-xl w-full h-[85vh] overflow-hidden flex flex-col p-1 gap-0 border-brand-blue/20 bg-brand-surface rounded-xl shadow-2xl">
          ...
          <div className="flex flex-col gap-4 relative z-10 w-full">
            ...
            {masterAccountProfile && (
              <div className="px-3 py-2 bg-brand-yellow rounded-lg border border-brand-yellow/20 flex items-center justify-between">
                <div className="flex flex-col">
                  <span className="text-[8px] font-black uppercase tracking-[0.2em] text-brand-blue/60 leading-none mb-1">Authenticated Master Authority</span>
                  <span className="text-xs font-black text-blue-400/90 tracking-tight">
                    {masterAccountProfile.organizationName || masterAccountProfile.displayName || "Allen 32"}
                  </span>
                </div>
              </div>
            )}
            ...
            <Button 
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setAllUserProfiles(items);
                  toast.success(`Manual sync: ${items.length} nodes`);
                } catch (err: any) {
            ...
            <div className="flex-1 overflow-y-auto">
              <table className="w-full table-fixed">
                <tbody className="divide-y divide-brand-blue/5">
                  {filteredUserProfiles
                    .filter(p => p.docId !== user?.email?.toLowerCase())
                    .map((profile) => (
                    <tr key={profile.uid} className="hover:bg-brand-blue/5 transition-colors h-14">
                      <td className="py-2 px-4 w-[80%]">
                        <div className="flex flex-col gap-0 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="font-bold text-xs text-brand-dark-grey truncate">
                              {profile.organizationName || profile.displayName || "Unregistered"}
                            </span>
                            {profile.email?.toLowerCase() === MASTER_ADMIN_EMAIL && (
                              <Badge className="bg-brand-blue text-white text-[7px] font-black px-1.5 h-3.5 rounded-sm uppercase tracking-tighter">
                                {profile.docId?.includes('@') ? 'UNIFIED MASTER' : 'LEGACY IDENTITY'}
                              </Badge>
                            )}
                            {filteredUserProfiles.filter(p => p.email === profile.email).length > 1 && (
                              <Badge className="bg-red-600 text-white text-[7px] font-black px-1.5 h-3.5 rounded-sm uppercase tracking-tighter animate-pulse shadow-sm shadow-red-200">
                                DUPLICATE DATA
                              </Badge>
                            )}
                            <Badge 
                          ...
                          <div className="flex items-center gap-2">
                             <span className="text-[10px] text-brand-grey font-medium truncate block pointer-events-none">
                               {profile.email}
                             </span>
                             <Badge variant="outline" className="text-[8px] font-mono text-brand-grey/50 px-1 py-0 h-4 border-brand-grey/10">
                               SECURE ID: {profile.docId || profile.uid}
                             </Badge>
                           </div>
                        </div>
                      </td>
                  ...
                  {filteredUserProfiles.length === 0 && (
                    <tr>
                      <td colSpan={3} className="h-32 text-center text-brand-grey/20 font-black uppercase text-[10px] tracking-widest">
                        Zero matches found
                      </td>
                    </tr>
                  )}
        ...
        <div className="p-3 px-4 bg-brand-blue/5 flex justify-between items-center border-t border-brand-blue/10">
          <div className="flex flex-col">
            <p className="text-[8px] text-brand-grey/40 uppercase tracking-widest font-black pointer-events-none">
              Authority: {user?.email}
            </p>
            <p className="text-[8px] text-brand-blue/40 uppercase tracking-widest font-black">
              {filteredUserProfiles.filter(p => p.email?.toLowerCase() !== MASTER_ADMIN_EMAIL).length} Total Registered Nodes | DB ID: {firebaseConfig.firestoreDatabaseId}
            </p>
          </div>
    ...
    <Dialog open={isDeleteConfirmOpen} onOpenChange={setIsDeleteConfirmOpen}>
      <DialogContent className="sm:max-w-[400px] bg-brand-surface border-brand-blue/20 shadow-2xl p-0 overflow-hidden rounded-2xl">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-brand-grey/40">
              ...
              <span className="text-brand-blue/80 font-bold">{nodeToDelete?.email}</span>
            </div>
            <div className="h-[1px] bg-brand-grey/5 w-full" />
            <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-brand-grey/40">
    ...
    <Dialog open={isLogoutConfirmOpen} onOpenChange={setIsLogoutConfirmOpen}>
      <DialogContent className="sm:max-w-[400px] bg-brand-surface border-brand-grey/20 shadow-2xl p-0 overflow-hidden rounded-2xl">
        <DialogHeader className="p-6 bg-brand-blue text-white relative">
          <div className="flex items-center gap-4 relative z-10">
            <div className="h-12 w-12 rounded-full bg-brand-yellow flex items-center justify-center shrink-0 shadow-lg relative overflow-hidden">
    ...
    <Dialog open={isEditMedDetailsOpen} onOpenChange={setIsEditMedDetailsOpen}>
      <DialogContent className="sm:max-w-[500px] bg-brand-surface border-brand-grey/20 shadow-2xl p-0 overflow-hidden rounded-2xl flex flex-col max-h-[90vh]">
        <DialogHeader className="p-6 bg-brand-blue text-white relative shrink-0">
          <div className="flex items-center gap-4 relative z-10 text-left">
            <div className="h-12 w-12 rounded-full bg-brand-yellow flex items-center justify-center shrink-0 shadow-lg relative overflow-hidden">
    ...
    <Dialog open={isResetConfirmOpen} onOpenChange={setIsResetConfirmOpen}>
      <DialogContent className="sm:max-w-[400px] bg-brand-surface border-brand-blue/20 shadow-2xl p-0 overflow-hidden rounded-2xl">
        ...
        <div className="p-6 space-y-4">
          <div className="bg-brand-blue/5 p-4 rounded-xl border border-brand-blue/10 text-center">
            <p className="text-xs text-brand-blue font-bold leading-relaxed">
              WARNING: You are about to clear all substances and transaction logs for <span className="underline decoration-brand-yellow decoration-2 underline-offset-2">"{nodeToReset?.organizationName || nodeToReset?.displayName}"</span>. The organization node itself will remain in the registry.
            </p>
          </div>
        </div>
        <DialogFooter className="flex flex-col sm:flex-row gap-2 p-6 bg-brand-blue/5 border-t border-brand-blue/10">
